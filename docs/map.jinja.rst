...
We will see a quick setup to configure the ``tool_mas`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_mas`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_mas`` formulas under ``/srv/salt/tool_mas/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_mas/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_mas/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_mas/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_mas/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_mas`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_mas`` formulas under ``/srv/salt/tool_mas/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_mas/parameters/roles
...
We create a configuration for the role ``tool_mas/server`` in ``/srv/salt/tool_mas/parameters/roles/tool_mas/server.yaml``:
...
We create another configuration for the role ``tool_mas/client`` in ``/srv/salt/tool_mas/parameters/roles/tool_mas/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_mas/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_mas/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_mas/parameters/dns:domain/  '/srv/salt/tool_mas/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-mas-as-json:
      test.nop:
        - name: {{ mas | json }}
...
.. code-block:: sls
    ...
    mas-config-file-file-managed:
      file.managed:
        - name: {{ mas.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='mas-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``mas`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ mas['winner'] }}
